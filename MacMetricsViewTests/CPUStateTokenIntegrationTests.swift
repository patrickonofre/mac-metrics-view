...
final class CPUStateTokenIntegrationTests: XCTestCase {
    ...
    // MARK: - Gemini provider (Phase 5, task_05)
    ...
    func testGeminiIngestRoutesToGeminiStoreOnly() {
        let state = CPUState(userDefaults: makeUserDefaults())
        state.update(provider: .gemini, with: [geminiEvent(input: 55, reasoning: 5)])

        XCTAssertEqual(state.tokenStores[.gemini]?.events.count, 1)
        XCTAssertEqual(state.tokenStores[.claude]?.events.count, 0)
        XCTAssertEqual(state.tokenStores[.codex]?.events.count, 0)
    }

    func testCombinedAggregateSumsAllThreeProviders() {
        let state = CPUState(userDefaults: makeUserDefaults())   // default selection: combined
        state.update(provider: .claude, with: [event(input: 100)])
        state.update(provider: .codex, with: [codexEvent(input: 40, reasoning: 5)])
        state.update(provider: .gemini, with: [geminiEvent(input: 25, reasoning: 3)])

        XCTAssertEqual(state.tokenProvider, .combined)
        XCTAssertEqual(state.tokenAggregate.input, 165)
        XCTAssertEqual(state.tokenAggregate.reasoning, 8)

        state.setTokenProvider(.gemini)
        XCTAssertEqual(state.tokenAggregate.input, 25)
        XCTAssertEqual(state.tokenAggregate.reasoning, 3)
    }

    func testGeminiIngestLeavesClaudeLedgerUntouched() {
        let state = CPUState(userDefaults: makeUserDefaults())

        state.update(provider: .gemini, with: [geminiEvent(input: 500_000)])

        XCTAssertTrue(state.tokenDailyLedger.days.isEmpty)   // rate-limit ledger is Claude-only
    }

    func testGeminiResetKeyPersistsIndependently() {
        let userDefaults = makeUserDefaults()
        let state = CPUState(userDefaults: userDefaults)
        state.update(provider: .gemini, with: [geminiEvent(input: 40)])

        state.setTokenProvider(.gemini)
        state.resetTokenCounter()

        XCTAssertNotNil(userDefaults.object(forKey: "CPUState.tokenResetAt.gemini"))
        XCTAssertNil(userDefaults.object(forKey: "CPUState.tokenResetAt.claude"))
    }

    func testGeminiCostUsesGeminiPriceTable() {
        let state = CPUState(userDefaults: makeUserDefaults())
        state.setTokenProvider(.gemini)
        // gemini-2.5-pro: 1M input × $1.25/MTok = $1.25.
        state.update(provider: .gemini, with: [geminiEvent(input: 1_000_000)])
